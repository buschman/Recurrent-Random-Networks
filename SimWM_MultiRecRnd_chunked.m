function [r_rec, r_rnd, t, opts] = SimWM_MultiRecRnd_chunked(varargin),
...
opts.UseChunk = 1; %whether to use chunked representation
opts.ChunkCenters = [3*pi/2 pi/2 NaN NaN]; %cell array of chunk centers
opts.ChunkWidths = pi/50; %width of chunk representations (what portions of center to pull out)
opts.ChunkPercentRemap = 0.5; %percent of connections to remap (drops as a function of width)

%Input vector into recurrent network
opts.InputBaseline = 0; %strength of random inputs
opts.InputCenter = rand(1, opts.N_rec_pools)*2*pi;
opts.InputWidth = pi/16*ones(1, opts.N_rec_pools);
...
%Remap inputs into neural space
ang = 2*pi*[1:opts.N_rec]./opts.N_rec;
for i = 1:length(opts.InputCenter),
    [~, opts.InputCenter(i)] = min(abs(mod(ang - opts.InputCenter(i) + pi, 2*pi) - pi));
    opts.InputWidth(i) = round(opts.InputWidth(i)./(2*pi/opts.N_rec));
end
...
W_rec_rnd_exists = false(opts.N_rec*opts.N_rec_pools, opts.N_rnd);
for i = 1:opts.N_rec*opts.N_rec_pools,
    ...
    W_rec_rnd_exists(i, temp_con) = true;
...
%Perform 'chunking' by remapping the connections between recurrent and
%random networks
if opts.UseChunk,
    for cur_chunk = 1:size(opts.ChunkCenters, 1),
        %Target random connections for this chunk
        chunk_rnd_ind = [1:round(opts.RndRec_f*opts.N_rnd*opts.ChunkPercentRemap)] + (cur_chunk-1)*round(opts.RndRec_f*opts.N_rnd*opts.ChunkPercentRemap);
        inv_chunk_rnd_ind = setdiff(1:opts.N_rnd, chunk_rnd_ind);
        
        %What recurrent neurons are in this 'chunk'? (within 3 STD)
        chunked_rec = false(opts.N_rec*opts.N_rec_pools, 1);
        for cp = 1:opts.N_rec_pools,
            if isnan(opts.ChunkCenters(cur_chunk, cp)), continue; end
            pool_ind = find(abs(ang - opts.ChunkCenters(cur_chunk, cp)) <= 3*opts.ChunkWidths);
            chunked_rec(pool_ind + (cp-1)*opts.N_rec) = true;
        end
        
        %Loop through neurons, moving connections
        for i = 1:opts.N_rec*opts.N_rec_pools,
            %Only process chunked locations
            if ~chunked_rec(i), continue; end 
            
            %Check whether we are done reshuffling
            my_prob = opts.ChunkPercentRemap*normpdf(ang(mod(i-1, opts.N_rec)+1) - opts.ChunkCenters(cur_chunk, floor((i-1)./opts.N_rec) + 1), 0, opts.ChunkWidths)./normpdf(0, 0, opts.ChunkWidths);
            while sum(W_rec_rnd_exists(i, chunk_rnd_ind))/sum(W_rec_rnd_exists(i, :)) < my_prob,
                %Find a putative connection to move
                source_ind = inv_chunk_rnd_ind(randsample(find(W_rec_rnd_exists(i, inv_chunk_rnd_ind)), 1));
                target_ind = chunk_rnd_ind(randsample(find(~W_rec_rnd_exists(i, chunk_rnd_ind)), 1));
                
                %Find a matching connection to move
                match_source_ind = randsample(find(W_rec_rnd_exists(~chunked_rec, target_ind)), 1);
                
                %Swap them both
                source_val = W_rec_rnd(i, source_ind);
                target_val = W_rec_rnd(i, target_ind);
                source_match_val = W_rec_rnd(match_source_ind, source_ind);
                target_match_val = W_rec_rnd(match_source_ind, target_ind);
                W_rec_rnd(i, target_ind) = source_val;
                W_rec_rnd(i, source_ind) = target_val;
                W_rec_rnd(match_source_ind, source_ind) = target_match_val;
                W_rec_rnd(match_source_ind, target_ind) = source_match_val;
                
                %Do it for other direction (keep symmetric)
                source_val = W_rnd_rec(source_ind, i);
                target_val = W_rnd_rec(target_ind, i);
                source_match_val = W_rnd_rec(source_ind, match_source_ind);
                target_match_val = W_rnd_rec(target_ind, match_source_ind);
                W_rnd_rec(target_ind, i) = source_val;
                W_rnd_rec(source_ind, i) = target_val;
                W_rnd_rec(source_ind, match_source_ind) = target_match_val;
                W_rnd_rec(target_ind, match_source_ind) = source_match_val;
                
                %Remap the existing connections
                W_rec_rnd_exists(i, target_ind) = true;
                W_rec_rnd_exists(i, source_ind) = false;
                W_rec_rnd_exists(match_source_ind, source_ind) = true;
                W_rec_rnd_exists(match_source_ind, target_ind) = false;
            end %loop moving connections around
            
        end %loop through recurrent neurons
    end %chunk loop
end %use chunks?
