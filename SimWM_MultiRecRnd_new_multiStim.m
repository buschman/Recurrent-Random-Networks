function SimWM_MultiRecRnd_new_multiStim(varargin),
...
my_save_fn = sprintf('SimWM_f%04.0f_TargetFR%03.0f_MaxFR%03.0f_RecPosWidth%03.0f%s.mat', ...
    opts.RndRec_f*1000, opts.RecToRndW_TargetFR*100, opts.MaxAvgRndFR, opts.RecWPositiveWidth*100, opts.FileAppend);

if exist(my_save_fn, 'file') & ~opts.Overwrite,
...
for cur_stim = 1:opts.NumStims,
    ...
    out_t = [0:opts.Output_dt:opts.SimTime];
    temp_r_rec = NaN*ones(size(r_rec, 1), size(r_rec, 2), length(out_t));
    ...
    temp_r_rnd = NaN*ones(size(r_rnd, 1), length(out_t));
    ...
    if opts.RunningPlot,
        ...
        for i = 1:opts.N_rec_pools,
            ...
            hold all;
            v = axis;
            plot(opts.InputTime(1, 1, 1)*[1 1], v(3:4), 'w-'); plot(opts.InputTime(2, 1, 1)*[1 1], v(3:4), 'w-');
