function spockSimWM_RecRnd_MultiTrials(my_task_id, num_trials, num_fullsave_trials, overwrite)
...
length(sim.RndRec_f)*length(sim.RecToRndW_TargetFR)*length(sim.MaxAvgRndFR)*length(sim.RecWPositiveWidth)*length(sim.NumInputs)
...
my_save_fn = sprintf('SimWM_f%04.0f_TargetFR%03.0f_MaxFR%03.0f_RecPosWidth%03.0f_Inputs%1.0f.mat', ...
    mysim.RndRec_f*1000, mysim.RecToRndW_TargetFR*100, mysim.MaxAvgRndFR, mysim.RecWPositiveWidth*100, mysim.NumInputs);
...
for cur_trial = 1:num_trials,
    [r_rec, r_rnd, t, opts] = SimWM_MultiRecRnd_new('RndRec_f', mysim.RndRec_f, 'RecToRndW_TargetFR', mysim.RecToRndW_TargetFR, ...
