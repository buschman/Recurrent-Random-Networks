function spockSimWM_RecRnd_MultiTrials_att_fullWeightandF(my_task_id, num_trials, num_fullsave_trials, overwrite)
...
sim.NumInputs = [1 2 4 6 8];

length(sim.RndRec_f)*length(sim.W_ratio)*length(sim.NumInputs)
...
my_save_fn = sprintf('SimWM_f%04.0f_Wratio%s_Inputs%1.0f.mat', ...
    mysim.RndRec_f*1000, strtrim(strrep(rats(mysim.W_ratio), '/', '-')), mysim.NumInputs);
...
for cur_trial = 1:num_trials,
    ...
    [r_rec, r_rnd, t, opts] = SimWM_MultiRecRnd_new_att('SimTime', 1.5, 'RunningPlot', 0, ...
        'RndRec_f', mysim.RndRec_f, 'InputTime', cur_inp_time, 'N_rnd', 1024, ...
    ...
    temp_vect = squeeze(nanmean(double(r_rec).*repmat(exp(-sqrt(-1)*neuron_angs), [1 opts.N_rec_pools length(t)]), 1))';
    mem_ang = angle(temp_vect);
    mem_len = abs(temp_vect);
