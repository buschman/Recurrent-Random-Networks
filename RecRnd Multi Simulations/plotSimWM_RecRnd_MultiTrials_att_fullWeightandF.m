function plotSimWM_RecRnd_MultiTrials_att_fullWeightandF(my_task_id)
...
my_save_dir = sprintf('Plots for SimWM_Wratio%s', ...
    strtrim(strrep(rats(mysim.W_ratio), '/', '-')));
...
for cur_f_ind = 1:length(sim.RndRec_f),
    ...
    for cur_input_ind = 1:length(sim.NumInputs),
        ...
        my_save_fn = sprintf('SimWM_f%04.0f_Wratio%s_Inputs%1.0f.mat', ...
            cur_f*1000, strtrim(strrep(rats(mysim.W_ratio), '/', '-')), cur_input);
...
figure;
plot(sim.RndRec_f, squeeze(nanmean(ovr_vm_circvar_good((t >= 0.3) & (t < 0.8), :, :), 1))');
%set(gca, 'XTick', [1:length(sim.RndRec_f)], 'XTickLabel', sim.RndRec_f);
...
for i = 1:length(sim.NumInputs),
    ...
    plot(sim.RndRec_f, squeeze(nanmean(ovr_vm_good_prct((t >= 0.3) & (t < 0.8), :, :, i)))');
    xlabel('Connectivity fraction'); ylabel('Percent ''Remembered''');
    legend(recpool_leg_str);
    title(sprintf('Effect of Connectivity on Mnemonic Accuracy (Von Mises %% Remembered; based on VectLen) - %d Items Stored', sim.NumInputs(i)));
end
