function spockSimWM_RecRnd_MultiTrials_att(my_task_id, num_trials, num_fullsave_trials, overwrite)
...
if ispc,
    ...
    addpath('B:\Projects\Models of Working Memory\Recurrent-Random Networks');
elseif isunix,
    base_dir = '/jukebox/buschman/Projects/Models of Working Memory/Recurrent-Random Networks/RecRnd Multi Simulations';
    addpath('/jukebox/buschman/Projects/Models of Working Memory/Recurrent-Random Networks');
end
cd(base_dir);

%% Convert task id into parameters setting
sim.RndRec_f = [0 0.050 0.100 0.150 0.200 0.250 0.300 0.400 0.500 0.750 1.000];
sim.W_ratio = [1/3 1/2 2/3 3/4 1 4/3 3/2 2/1 3/1];
...
length(sim.RndRec_f)*length(sim.RecToRndW_TargetFR)*length(sim.RndToRecW_TargetFR)*length(sim.RecWPositiveWidth)*length(sim.NumInputs)*length(sim.N_rnd)

mysim.RndRec_f = sim.RndRec_f(mod(my_task_id, length(sim.RndRec_f))+1);
my_task_id = floor(my_task_id./length(sim.RndRec_f));
mysim.W_ratio = = sim.RndRec_f(mod(my_task_id, length(sim.W_ratio))+1);
...
my_save_fn = sprintf('SimWM_Nrnd%04.0f_f%04.0f_TargetFR%03.0f_%03.0f_RecPosWidth%03.0f_Inputs%1.0f.mat', ...
    mysim.N_rnd, mysim.RndRec_f*1000, mysim.RecToRndW_TargetFR*100, mysim.RndToRecW_TargetFR*100, ...
    mysim.RecWPositiveWidth*100, mysim.NumInputs);
...
cur_inp_time = repmat([0.1 0.2]', [1 8]); %in seconds
cur_inp_time(:, (mysim.NumInputs+1:end)) = NaN; %only drive the first cur_inp pools

cur_att_time = repmat([0.9 1.1]', [1 8]);
cur_att_inp = 3*[7 -1 -1 -1 -1 -1 -1 -1];

fprintf('\nStarting simulation...\n'); start_time = now;
for cur_trial = 1:num_trials,
    [r_rec, r_rnd, t, opts] = SimWM_MultiRecRnd_new_att('N_rnd', mysim.N_rnd, 'RndRec_f', mysim.RndRec_f, ...
        'RecToRndW_TargetFR', mysim.RecToRndW_TargetFR, 'RndToRecW_TargetFR', mysim.RndToRecW_TargetFR, ...
        'RecWPositiveWidth', mysim.RecWPositiveWidth, 'InputTime', cur_inp_time, ...
        'AttInputTime', cur_att_time, 'AttInputValue', cur_att_inp, 'SimTime', 1.5);
