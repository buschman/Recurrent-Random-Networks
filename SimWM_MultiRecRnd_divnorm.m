function [r_rec, r_rnd, t, opts] = SimWM_MultiRecRnd_divnorm(varargin),
...
opts.UseDivNorm = 1;
opts.DivNorm_Offset = 1;
opts.DivNorm_PopFact = 10^-3;
...
for i = 1:opts.N_rec_pools,
    for j = 1:opts.N_rec_pools,
        if i == j, continue; end(rand(opts.N_rec, opts.N_rec) - 0.5);
        W_rec_rec{i,j} = W_rec_rec{i,j}./(opts.N_rec.*(opts.N_rec_pools - 1));
    end
        W_rec_rec{i,j} = opts.PoolWBaseline + opts.PoolWRandom*2*
end
...
if opts.RunningPlot, line_fig = figure; time_fig = figure; end
for t_ind = 1:length(t),
    ...
    if opts.UseDivNorm,        
        g_rec = g_rec./(opts.DivNorm_Offset + opts.DivNorm_PopFact*(repmat(sum(g_rec, 1), [opts.N_rec 1]) - g_rec));
        g_rnd = g_rnd./(opts.DivNorm_Offset + opts.DivNorm_PopFact*(repmat(sum(g_rnd, 1), [opts.N_rnd 1]) - g_rnd));
    end
