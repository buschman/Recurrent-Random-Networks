function [r_rec, r_rnd, t, opts] = SimWM_MultiRecRnd_new_test(varargin),
...
opts.RecToRndW_TargetFR = 0.5; %weight scaling factor for connections from recurrent network to random network
opts.RecToRndW_EIBalance = -1; %-1 means inter-network weights are balanced (sum = 0)
...
opts.RndToRecW_TargetFR = 0.25; %weight scaling factor for connections from random network to recurrent networks
opts.RndToRecW_EIBalance = -1; %-1 means inter-network weights are balanced (sum = 0)
